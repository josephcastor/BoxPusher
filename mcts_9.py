...
class MonteCarloTreeSearch:
    ...
    def get_greedy_action(self, state):
        """
        When using heuristic based simulation, this function is used to return the next greedy action.
        """
        # Human:
...
class VacuumEnvironmentState:
    ...
    def get_legal_actions(self):
        ...
        if self.human.position <= len(self.belt) - 1 and not self.human.holding_box and self.belt[
            self.human.position] == 1:
        ...
        if self.robot.position <= len(self.belt) - 1 and not self.robot.holding_box and self.belt[
...
def simulate_mcts_run(planning_duration, steps_per_trial, heuristic, trial_num):
    ...
    while i < steps_per_trial:
        ...
        mcts = MonteCarloTreeSearch(root, planning_duration=planning_duration, heuristic=heuristic)
        ...
        next_state = current_state.take_action(best_action)
...
def run_experiments(planning_duration=3, steps_per_trial=30, heuristic=True, num_trials=50):
    results = []

    for n in range(1):
        # Run multiple trials for each grid size and store the data.
        trials_data = [simulate_mcts_run(planning_duration, steps_per_trial, heuristic, i) for i in range(num_trials)]
    ...
    if len(sys.argv) > 1:
        ...
        if "-t" in sys.argv[1:]:
            ...
            if len(sys.argv) <= index + 1:
                print("please provide a planning time argument")
                exit()
            else:
                planning_time = float(sys.argv[index + 1])
        if "-s" in sys.argv[1:]:
            index = sys.argv.index("-s")
            if len(sys.argv) <= index + 1:
                print("please provide a steps per trial time argument")
                exit()
            steps_per_trial = int(sys.argv[index + 1])


    run_experiments(planning_time, steps_per_trial, heuristic)
